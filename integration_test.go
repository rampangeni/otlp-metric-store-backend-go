...
func TestInsertGauge(t *testing.T) {
	...
	extracted := ExtractMetrics(resourceMetrics)
	if err := store.InsertMetadata(ctx, extracted.Metadata); err != nil {
		t.Fatalf("inserting metadata rows: %v", err)
	}
	if err := store.InsertGauge(ctx, extracted.GaugeRows); err != nil {
	...
	err := store.conn.QueryRow(ctx,
        ...
        JOIN otel_metric_metadata m ON g.MetricID = m.MetricID
...
func TestInsertSum(t *testing.T) {
	...
	if err := store.InsertSum(ctx, extracted.SumRows); err != nil {
	...
	err := store.conn.QueryRow(ctx,
        ...
        JOIN otel_metric_metadata m ON s.MetricID = m.MetricID
...
func TestGRPCToClickHouse(t *testing.T) {
	...
	colmetricspb.RegisterMetricsServiceServer(grpcServer, newServer("bufconn", store, NewMetricIdentityCache(1000)))
	go func() {
